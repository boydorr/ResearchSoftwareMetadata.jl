...
function crosswalk(git_dir = readchomp(`$(Git.git()) rev-parse --show-toplevel`);
    ...
    if haskey(project, "author_details")
        for author in project["author_details"]
            ...
            if haskey(author, "orcid")
                ...
                orcid_people[author["orcid"]] = person
                isnothing(person) &&
                    @error "Could not retrieve ORCID $(author["orcid"]) " *
                           "from orcid.org, falling back to Project.toml data"
            end
            if !isnothing(person)
    ...
    if !isempty(authors)
        ...
        for author in authors
            if author ∉ proj_authors
                ...
                elseif parse_author(author) ∈ parse_author.(proj_authors)
                    @info "Fixing format of $author in Project.toml"
                else
    ...
    for author in proj_authors
        person = haskey(author, "orcid") ?
                 get(orcid_people, author["orcid"], nothing) : nothing
        dict = OrderedDict{String, Any}("type" => "Person")
        if isnothing(person)
            if haskey(author, "name")
                given, family = split_name(author["name"])
                if isnothing(given)
                    dict["name"] = family
                else
        ...
        if haskey(author, "affiliation")
            ...
            for org in author["affiliation"]
                ...
                if haskey(org, "ror")
                    ...
                    if isnothing(info)
                        @error "Could not retrieve ROR $ror from ror.org, " *
                               "falling back to Project.toml data"
                        haskey(org, "name") && (d["name"] = org["name"])
                        d["identifier"] = "https://ror.org/" * ror
                    else
                        d["name"] = info["name"]
    ...
    if isempty(cm_authors)
        ...
        author_key(d) = get(d, "id",
                            (get(d, "givenName", ""),
                             get(d, "familyName", get(d, "name", ""))))
        keys_from_proj = author_key.(cm_from_proj)
        ...
        for dict in cm_from_proj
            if author_key(dict) ∉ keys_from_cm
                @info "Adding $(get(dict, "givenName", "")) " *
                      "$(get(dict, "familyName", get(dict, "name", ""))) " *
                      "to codemeta.json authors"
            end
        end
    end
    ...
    publications = reconcile!(project, codemeta, "publications",
                              ...
                              from_cm = refs -> replace.(refs isa Vector ?
                                                         refs : [refs],
                                                         "https://doi.org/" => ""))
    if !isnothing(publications)
        for doi in publications
            check_doi(doi) ||
                @error "DOI $doi in Project.toml publications does not resolve"
        end
    end
    ...
    for author in codemeta["author"]
        ...
        if haskey(author, "familyName") && haskey(author, "givenName")
            dict["name"] = "$(author["familyName"]), $(author["givenName"])"
        elseif haskey(author, "name")
            dict["name"] = author["name"]
        elseif haskey(author, "familyName")
            dict["name"] = author["familyName"]
        else
            @warn "Skipping codemeta author with no name in .zenodo.json: " *
                  "$author"
            continue
        end
        if haskey(author, "id")
    ...
    if !isnothing(publications)
        for doi in publications
            ...
            dict["scheme"] = "doi"
            dict["identifier"] = doi
            dict["relation"] = "isSupplementTo"
            push!(crosswalk_d["related_identifiers"], dict)
        end
    end
